        ...
        {orders.map((order) => {
          ...
          return (
            <div key={order.id} className="border border-gray-100 rounded-xl bg-white overflow-hidden shadow-sm transition-shadow hover:shadow-md">
              {/* Order Header */}
              <div
                className="p-4 sm:p-6 cursor-pointer bg-gray-50/50 hover:bg-gray-50 transition-colors select-none"
                onClick={() => toggleExpand(order.id)}
              >
                <div className="flex justify-between items-center gap-4 w-full">
                  {/* Left section: Order No and Date */}
                  <div className="sm:flex sm:items-center gap-4 sm:gap-6 space-y-2 sm:space-y-0 min-w-0">
                    <div>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Sipariş No</p>
                      <p className="font-mono text-xs sm:text-sm font-semibold text-charcoal">{orderNum}</p>
                    </div>
                    <div className="hidden sm:block w-px h-8 bg-gray-200"></div>
                    <div>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Sipariş Tarihi</p>
                      <p className="text-xs sm:text-sm font-semibold text-charcoal">{date}</p>
                    </div>
                  </div>

                  {/* Right section: Total and Status */}
                  <div className="flex flex-col items-end sm:items-start sm:flex-row sm:items-center gap-2 sm:gap-6 shrink-0">
                    <div className="text-right sm:text-left">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">Toplam</p>
                      <p className="text-xs sm:text-sm font-bold text-[#C9A84C]">{order.total.toLocaleString("tr-TR")} ₺</p>
                    </div>
                    <div className="hidden sm:block w-px h-8 bg-gray-200"></div>
                    <div className="flex items-center gap-2">
                      <span className={`text-[10px] sm:text-xs font-semibold px-2 sm:py-0.5 sm:px-3 sm:py-1 rounded-full border whitespace-nowrap ${statusInfo.color}`}>
                        {statusInfo.label}
                      </span>
                      <button
                ...
                <div className="p-4 sm:p-6 border-t border-gray-100">
                  ...
                  <div className="space-y-4">
                    {order.items?.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex items-center gap-3 sm:gap-4 py-3 border-b border-gray-50 last:border-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="text-xs sm:text-sm font-semibold text-charcoal truncate">{itemName}</h4>
                            <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">{item.size}</p>
                          </div>
                          <div className="text-right font-montserrat flex-shrink-0">
                            <p className="text-xs sm:text-sm font-bold text-charcoal">{item.unitPrice?.toLocaleString("tr-TR")} ₺</p>
                            <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">Adet: {item.quantity}</p>
                          </div>
                        </div>
                      );
                    })}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 pt-6">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2 rounded-lg text-sm font-medium transition-all ${
              currentPage === 1
