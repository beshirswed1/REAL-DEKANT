...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      {/* Profile Avatar Card */}
      <div className="flex flex-col sm:flex-row items-center text-center sm:text-left gap-4 sm:gap-6 p-5 sm:p-6 bg-gradient-to-r from-gray-50 to-white border border-gray-100 rounded-xl">
        <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#C9A84C] to-[#8B6914] flex items-center justify-center text-white text-2xl font-playfair font-bold shadow-md shrink-0">
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-xl font-playfair font-bold text-charcoal truncate">
            {user.displayName || "İsimsiz Kullanıcı"}
          </h3>
          <p className="text-sm text-gray-500 mt-1 truncate">{user.email}</p>
          {joinDate && (
            <p className="text-xs text-gray-400 mt-2 flex items-center justify-center sm:justify-start gap-1.5">
              <FiCalendar size={12} />
              Üyelik: {joinDate}
            </p>
          )}
          ...
          {isEditing ? (
            <div className="flex items-center gap-2 sm:gap-3">
              <input
                ...
                className="flex-1 min-w-0 px-3 py-2.5 sm:px-4 sm:py-3 bg-white border border-[#C9A84C]/40 rounded-lg focus:ring-2 focus:ring-[#C9A84C]/30 focus:border-[#C9A84C] transition-all outline-none text-sm"
              ...
              <button
                ...
                disabled={loading}
                className="h-10 w-10 sm:h-12 sm:w-12 p-2 sm:p-3 bg-[#C9A84C] text-white rounded-lg hover:bg-[#8B6914] transition-colors disabled:opacity-50 flex items-center justify-center shrink-0"
                aria-label="Kaydet"
              >
                {loading ? (
              ...
              <button
                ...
                className="h-10 w-10 sm:h-12 sm:w-12 p-2 sm:p-3 bg-gray-100 text-gray-600 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center shrink-0"
                aria-label="İptal"
              >
                <FiX size={18} />
              </button>
            ...
            <div className="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-charcoal text-sm">
