...
function cleanWhatsapp(phone: string): string {
  if (!phone) return "";
  ...
  if (cleaned.length === 13 && cleaned.startsWith("9005")) {
...
export default function CheckoutClient() {
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        if (data.contactWhatsapp) {
          const cleaned = cleanWhatsapp(data.contactWhatsapp);
  ...
  const submitOrderCreation = async () => {
    ...
    const triggerWhatsappRedirect = (finalOrderId: string) => {
      ...
      const cleanedNum = cleanWhatsapp(whatsappNumber);
      const url = `https://wa.me/${cleanedNum}?text=${encoded}`;
      try {
      ...
      if (data.fallbackToClient) {
        ...
        if (!finalUserId) {
          try {
            ...
            setServerError("Misafir girişi yapılamadı. Lütfen tekrar deneyin veya üye girişi yapın.");
  ...
  return (
    <div className="bg-cream-light min-h-screen py-6 sm:py-12 px-3 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        
        {/* Header Title */}
        {step !== 3 && (
          <div className="text-center mb-10 space-y-2">
            <h1 className="font-playfair text-2xl sm:text-3xl md:text-4xl font-bold tracking-wide uppercase text-charcoal">
              {t.title}
            </h1>
            <div className="h-0.5 w-12 bg-[#C9A84C]/45 mx-auto" />
          </div>
        ...
        {/* Step Indicator */}
        <div className="flex items-center justify-between max-w-md sm:max-w-lg mx-auto mb-8 sm:mb-16 relative px-2 sm:px-0">
          ...
          <div className="flex flex-col items-center z-10 space-y-2">
            <div
              className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center font-montserrat text-[10px] sm:text-xs font-bold transition-all duration-500 border ${
                step > 1
            ...
            <span className="font-montserrat text-[8px] sm:text-[10px] tracking-wider font-bold uppercase text-charcoal">
              {t.delivery}
            </span>
          </div>
        ...
        {step === 3 ? (
          /* ──────────────── STEP 3: CONFIRMATION ──────────────── */
          <div className="max-w-2xl mx-auto bg-white border border-[#C9A84C]/25 shadow-xl p-5 sm:p-8 md:p-12 text-center space-y-6 sm:space-y-8 animate-fade-in">
            ...
            <div className="space-y-3">
              <h2 className="font-playfair text-xl sm:text-2xl md:text-3xl font-bold tracking-wide uppercase text-charcoal">
                {t.successTitle}
              </h2>
            ...
            {paymentMethod === "whatsapp" && (
              <div className="bg-[#FAF7F0] border border-[#25D366]/25 p-6 rounded-sm space-y-4 max-w-md mx-auto my-6 shadow-sm animate-fade-in">
                ...
                <a
                  href={`https://wa.me/${cleanWhatsapp(whatsappNumber)}?text=${encodeURIComponent(
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-start">
            ...
            <div className="lg:col-span-8 bg-white border border-[#C9A84C]/15 shadow-lg p-4 sm:p-6 md:p-8 rounded-sm">
            ...
            <aside className="lg:col-span-4 lg:sticky lg:top-24 bg-white border border-[#C9A84C]/25 shadow-lg p-4 sm:p-6 rounded-sm">
