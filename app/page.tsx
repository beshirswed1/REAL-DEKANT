...
export default async function HomePage() {
  ...
  try {
    const [productsRes, configDoc] = await Promise.allSettled([
      getDocuments<Product>(COLLECTIONS.PRODUCTS, [
    ...
    if (productsRes.status === "fulfilled") {
      ...
      console.warn("Firestore products fetch failed:", productsRes.reason);
    }

    if (configDoc.status === "fulfilled" && configDoc.value.exists) {
      settingsConfig = JSON.parse(JSON.stringify(configDoc.value.data())) as SiteConfig;
    }
  } catch (error) {
  ...
  return (
    <div className="flex flex-col bg-[#FAF7F0] text-[#1A1A1A] min-h-screen overflow-x-hidden">
      ...
      {settingsConfig?.showBestSellers !== false && bestSellers.length > 0 && (
        <section id="best-sellers" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
      ...
      {settingsConfig?.showWhyDekant !== false && (
        <section className="bg-[#0D0D0D] border-t border-[#C9A84C]/15 text-[#F8F3E8] py-20 sm:py-28 relative overflow-hidden">
          {/* Subtle radial glow */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <div className="space-y-7 order-2 lg:order-1">
              ...
              {/* Headline */}
              <h2 className="font-playfair text-2xl sm:text-3xl lg:text-4xl font-bold tracking-wide uppercase text-[#F8F3E8] leading-tight">
