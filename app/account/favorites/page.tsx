...
export default function FavoritesPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 px-4 text-center animate-in fade-in duration-700">
        <div className="relative w-28 h-28 mb-8 flex items-center justify-center">
          ...
          <FiHeart size={45} className="text-[#C9A84C] relative z-10 drop-shadow-sm" />
        </div>
        <h2 className="text-3xl font-playfair font-bold text-charcoal mb-4">Favori Listeniz Boş</h2>
        <p className="text-charcoal/60 font-montserrat text-sm max-w-md mb-10 leading-relaxed">
          Koleksiyonumuzdaki eşsiz kokuları keşfedin ve kendi imza koleksiyonunuzu oluşturmaya başlayın.
        </p>
        <Link 
          href="/shop"
          className="group relative inline-flex items-center justify-center bg-charcoal text-white font-montserrat text-xs tracking-[0.25em] uppercase py-4 px-12 overflow-hidden transition-all duration-500 shadow-[0_5px_15px_rgba(0,0,0,0.1)] hover:shadow-[0_5px_25px_rgba(201,168,76,0.3)] hover:border-[#C9A84C] border border-transparent"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
        {items.map((item) => (
          <div key={item.productId} className="group flex flex-col bg-[#fdfbf7] border border-[#C9A84C]/20 hover:border-[#C9A84C]/60 transition-all duration-500 hover:shadow-[0_10px_40px_rgba(201,168,76,0.15)] relative">
            ...
            {/* Action Buttons */}
            <div className="absolute top-3 right-3 z-20 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-x-2 group-hover:translate-x-0">
              <button
                onClick={() => handleRemove(item.productId)}
                className="p-2.5 bg-white/90 backdrop-blur-md rounded-full text-charcoal hover:text-red-500 hover:bg-white transition-all shadow-md"
            ...
            <Link href={`/shop/${item.slug}`} className="relative aspect-[4/5] w-full bg-cream-light overflow-hidden">
              {item.image ? (
                ...
                <div className="w-full h-full flex items-center justify-center bg-cream-dark/20 text-charcoal/40 font-montserrat text-xs tracking-widest uppercase">
                  Görsel Yok
                </div>
              )}
            ...
            {/* Content Container */}
            <div className="p-5 flex flex-col flex-grow bg-white z-10 transition-transform duration-500 group-hover:-translate-y-2">
              <span className="text-[10px] font-montserrat tracking-[0.25em] text-[#C9A84C] uppercase font-bold mb-2">
                {item.brand}
              </span>
              <Link href={`/shop/${item.slug}`} className="font-playfair text-lg font-bold text-charcoal mb-3 group-hover:text-[#8B6914] transition-colors line-clamp-2 leading-tight">
                {item.perfumeName}
              </Link>
              
              <div className="mt-auto pt-4 flex items-center justify-between border-t border-[#C9A84C]/10">
                <div className="flex flex-col">
                  <span className="font-montserrat text-xs text-charcoal/50 mb-0.5">Başlangıç Fiyatı</span>
                  <span className="font-montserrat text-[15px] font-bold text-charcoal">
                    ₺{item.price.toLocaleString("tr-TR")}
                  </span>
                </div>
                <button
                  onClick={() => handleAddToCart(item)}
                  className="p-3 bg-charcoal text-[#C9A84C] hover:bg-[#C9A84C] hover:text-black transition-all duration-300 shadow-sm"
