...
async function fetchProduct(slug: string): Promise<Product | null> {
  try {
    ...
    console.warn("Firestore fetch error on detail page, using mock fallback:", error);
  }
...
export default async function ProductDetailPage({
  ...
  // Load all products to compute similar items
  let allProducts: Product[] = [];
  try {
    allProducts = await getDocuments<Product>(COLLECTIONS.PRODUCTS, [
      where("isPublished", "==", true),
    ]);
  } catch {
  ...
  return (
    <div className="bg-cream-light min-h-screen py-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-10 gap-10 lg:gap-16 items-start">
          ...
          <div className="md:col-span-4 w-full space-y-6">
            <div className="space-y-3">
              ...
              <h1 className="font-playfair text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-wide text-[#C9A84C]">
                {product.perfumeName}
              </h1>
            </div>
        ...
        <div className="border-t border-[#C9A84C]/15 pt-16 grid grid-cols-1 lg:grid-cols-2 gap-12 sm:gap-16 items-start">
